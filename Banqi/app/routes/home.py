...
def home():
    ...
    players = Player.query.filter_by(user_id=user_id).order_by(Player.id.desc()).limit(20).all()
    seen = set()
    for p in players:
        game = p.game
        ...
        opp_line_a = f"{pa['name'] or 'ANON_USER'} ({pa['elo_p'] if pa['elo_p'] is not None else '1200'})"
        opp_line_b = f"{pb['name'] or 'ANON_USER'} ({pb['elo_p'] if pb['elo_p'] is not None else '1200'})"
